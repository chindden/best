/*
爱奇艺会员签到脚本 (整合 V7 升级礼包版)

更新时间: 2025/12/24
脚本兼容: QuantumultX, Surge4, Loon, JsBox, Node.js
原作者: NobyDa
修改: 增加 V7 会员领取白金 VIP 时长/积分逻辑
...
function VipGrowth() {
  return new Promise(resolve => {
    // 1. 构造 POST 请求体 (表单格式 application/x-www-form-urlencoded)
    // 对应 Python 里的 data={"code": "...", "P00001": ...}
    const requestBody = `code=k8sj74234c683f&P00001=${P00001}`;
    ...
    const URL = {
      ...
      headers: {
        'Cookie': cookie,
        'Content-Type': 'application/x-www-form-urlencoded' // 必须指定这个 Content-Type
      },
      body: requestBody
    }
    ...
    $nobyda.post(URL, function (error, response, data) {
      ...
      try {
        ...
        // 尝试解析返回的 JSON
        // 成功通常返回: {"code":"A00000", "msg":"成功", ...}
        // 失败通常返回: {"code":"A00002", "msg":"已领取", ...} 或 {"msg":"参数错误"}
        const obj = JSON.parse(data);
        
        if (obj.code === "A00000") {
          GrowthMsg = `V7升星钻: 领取成功 🎉`;
        } else {
          // 如果 msg 包含 "已领取" 或 "不满足"，只记录日志不弹窗干扰
          const msg = obj.msg || "未知错误";
          if (msg.includes("已领取") || msg.includes("上限") || obj.code === "A00002") {
             console.log(`爱奇艺-V7升星钻222: ${msg}`);
             resolve();
             return;
          }
          GrowthMsg = `V7升星钻: ${msg} (${obj.code}) ⚠️`;
        }
      } catch (e) {
        GrowthMsg = `V7升星钻: 解析错误 ${e.message}`;
      }
      
      pushMsg.push(GrowthMsg);
      console.log(`爱奇艺-${GrowthMsg} ${Details}`);
      resolve();
    })
  })
}
